...
const ShareModal: FC<Props> = (props: Props) => {
    ...
    const Icons = ICONS.slice(iconStartIdx, VISIBLE_ICONS_COUNT + iconStartIdx).map((icon, index) => (
        <a
            key={icon.href + index}
            href={icon.href}
            target={'_blank'}
        ...
        >
            <Image src={icon.svg} alt={icon.href + index} className={`min-w-[2.5rem] rounded-full`}/>
        </a>
    ));
