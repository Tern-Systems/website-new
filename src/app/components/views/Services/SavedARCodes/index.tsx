...
const SavedARCodes: FC = () => {
    const [menuData, setMenuData] = useState<MenuData>({arCode: null, x: 0, y: 0, isOpened: false});
    const [codeId, setCodeId] = useState<string | null>(null);

    const SavedCodes: ReactElement[] = SAVED_CODES.map((arCode, index) => {
        const id = (arCode.name + index).toLowerCase().split(' ').join('-');
        return (
            <div key={id} id={id}>
