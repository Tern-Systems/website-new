...
import {Button, ButtonIcon} from "@/app/ui/form";
...
const renderTable = (table: TableSection, navigate?: (link: string) => void) => {
    const TableItems: ReactElement[] = table.data.map((entry, idx) => (
        <tr key={entry.name.slice(5) + idx} className={'hover:bg-control2'}
        >
