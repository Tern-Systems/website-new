    ...
    const renderToggle = () => {
        ...
        return (
            <Button
                ...
                className={`box-content rounded-xxs px-3xs py-[0.3rem] font-oxygen text-basic ${parentEditId ? '!bg-[#0C545C] !text-[#ECF0F3]' : ''} ${!isDisabled ? 'hover:bg-blue hover:text-primary' : ''} ${classNameToggle} ${editState ? '[&]:bg-blue' : 'text-gray [&]:bg-white-d0'}`}
            >
        ...
        case 'image':
            ...
            Form = (
                <>
                    ...
                    <span className={`mt-xxs block text-section-xs ${waring ? '' : 'hidden'}`}>{waring}</span>
                    <span
                        className={`h-h-button-n mt-[min(1.3dvw,0.95rem)] flex gap-x-[min(1dvw,0.75rem)] text-section font-bold`}
                    >
                        {CancelBtn}
                        <Button
                            type={'button'}
