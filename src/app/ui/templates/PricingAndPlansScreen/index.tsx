...
const PricingAndPlansScreen: FC<Props> = (props: Props) => {
  ...
  const renderColumn = (
    ...
    const Benefits: ReactElement[] = benefitsData.map((benefit, subIndex) => {
      ...
      return (
        <li key={type + subIndex} className={`list-inside ${listImage}`}>
    ...
    const CollapsedContentSm = (
      <>
        <h2
          className={
            "flex mb-[min(2.7dvw,0.95rem)] font-oxygen text-header font-bold capitalize"
          }
        >
          {data?.icon ? (
            <Image
              src={data.icon}
              alt={type + " icon"}
              className={"mr-[min(1.1dvw,0.32rem)]"}
            />
          ) : (
            "--"
          )}
        ...
        <div
          className={
            "text-secondary mb-[min(4dvw,2.2rem)] text-[min(4.8dvw,1.25rem)]"
          }
        >
    ...
    return (
      <Collapsible
        ...
        classNameWrapper={
          "[&]:w-[min(90dvw,25rem)] border-small border-control-white-d0 sm:border-none text-left"
        }
      >
        {CollapsedContentSm}
        <ul
          className={
            "flex flex-col gap-[min(5.3dvw,1.5rem)] mt-[min(5.3dvw,1.5rem)] items-start"
          }
        >
          {Benefits}
        </ul>
        <div
          className={
            "flex flex-col mt-[min(8dvw,2.1rem)] font-oxygen text-[min(2.6dvw,var(--fz-note-))] text-secondary flex-grow justify-end"
          }
        >
          {Links}
        </div>
  ...
  return (
    <div className={"flex flex-col my-auto sm:my-0"}>
      <div
        className={
          "flex place-self-center p-[0.2rem] border-small rounded-full text-small"
        }
      >
        {Switch}
      </div>
      <div
        className={
          "flex place-self-center gap-[min(2.6dvw,4.13rem)] mt-[min(5.3dvw,3.12rem)] sm:flex-col"
        }
      >
        {renderColumns()}
      </div>
