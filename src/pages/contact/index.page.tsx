...
const ContactsPage: FC = () => {
    ...
    const InputField = ({
        ...
        <div key={field} className="text-[2.5rem] md:text-heading-l  sm:text-[2rem]">
            <label htmlFor={field} className="block mb-xs  md:mb-[1.125rem]  sm:mb-xxs">
                {label}{required ? '*' : ''}
            </label>
            <input
                type={type}
                ...
                onChange={onChange}
                required={required}
                className={cn('bg-control-gray-l0 border border-control-white w-full px-xs outline-none focus:ring-2 focus:ring-blue-500',
                    'text-primary ',
                    'md:x-[text-[2rem],px-xs]',
                    'sm:x-[text-heading,px-3xs]',
                )}
            />
        </div>
    );
                        ...
                        <form onSubmit={handleSubmit} className="relative z-10">
                            <div className="grid gap-16 md:gap-12 sm:gap-10">
                                ...
                                <div className="grid grid-cols-1 gap-4">
                                    <div className="text-[2.5rem] md:text-heading-l  sm:text-[2rem]">
                                        <label htmlFor="message" className="block mb-xs  md:mb-[1.125rem]  sm:mb-xxs">Message*</label>
                                        <textarea
                                            id="message"
                                            name="message"
                                            value={formData.message}
                                            onChange={handleChange}
                                            className={cn('bg-control-gray-l0 border border-control-white text-heading-l min-h-[25rem] w-full p-xs text-primary  outline-none focus:ring-2 focus:ring-blue-500',
                                                'md:x-[text-[2rem],p-xs]',
                                                'sm:x-[text-heading,p-3xs]'
                                            )}
                                        ></textarea>
                                    </div>
                                </div>
                                <button
                                    type="submit"
                                    className="bg-black max-w-[7.9375rem] border border-control-gray-l0  px-6 py-3 text-heading-s hover:bg-blue-700 transition text">
                                    Submit
                                </button>
                            </div>
                        </form>
