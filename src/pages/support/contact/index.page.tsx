...
const ContactsPage: FC = () => {
  ...
  return (
    <>
      ...
      <section className="sticky top-0 z-0 flex justify-center w-full">
        <div
          className="relative w-full h-dvh max-h-[62.5rem] max-w-[120rem] bg-cover bg-center"
          style={{ backgroundImage: `url(${OFFICE_GIRL_3.src})` }}
        >
          <div className={cn(styles.content, 'relative z-10 flex items-start justify-start')}>
            <h1
              className={cn(
                'w-min text-left leading-n mb-n',
                'text-96 lg:mt-6xl-1',
                'md:mt-xl lg:text-96',
                'sm:flex sm:mt-xs text-64'
              )}
            >
              Contact Tern
            </h1>
      ...
      {/* Main content */}
      <div className="relative z-10 bg-black bg-gradient-to-b from-blue from-0% to-black to-5% lg:to-10%">
        <section
          className={cn(
            styles.content,
            'relative z-10',
        ...
        >
          ...
          <div className="lg:w-1/2 md:w-[65%]">
            <h2 className="mb-6xl-1 text-left text-48 font-[500] sm:mb-xl">Get in Touch</h2>
            <form onSubmit={handleSubmit} className="relative z-10 [&_*]:tracking-wide">
              <div className="grid gap-n">
                <div className="grid grid-cols-2 gap-n sm:grid-cols-1">
                  <Input value={formData.firstName} onChange={setFormValue('firstName')} required {...INPUT_PROPS}>
                    First Name*
                  </Input>
                  <Input value={formData.lastName} onChange={setFormValue('lastName')} required {...INPUT_PROPS}>
                    Last Name*
                  </Input>
                </div>
                <div className="grid grid-cols-1">
                  <Input value={formData.company} onChange={setFormValue('company')} {...INPUT_PROPS}>
                    Company
                  </Input>
                </div>
                <div className="grid grid-cols-2 gap-n sm:grid-cols-1">
                  <Input type="email" value={formData.email} onChange={setFormValue('email')} {...INPUT_PROPS}>
                    Email*
                  </Input>
                  <Input type="phone" value={formData.phone} onChange={setFormValue('phone')} {...INPUT_PROPS}>
                    Phone
                  </Input>
                </div>
                <div className="grid grid-cols-1">
                  <Input type="textarea" value={formData.message} onChange={setFormValue('message')} required {...INPUT_PROPS}>
                    Message*
                  </Input>
                </div>
                <Input
                  ...
                  isCustomCheckbox
                >
                  May Tern provide you with personalized communications about Tern and select Tern-partner products, services, offers and events?
                </Input>
                <Button type="submit" className="border-control-gray-l0 max-w-[7.9375rem] border rounded-none bg-black px-6 py-3 text-21">
                  Submit
                </Button>
              </div>
            </form>
          ...
          <div className="lg:w-1/2 md:w-[65%] flex">
            <div className="h-full w-full md:h-[60dvh] md:max-h-[43.5rem] sm:h-[70dvh] sm:max-h-[29.75rem]">
              <MapEmbed />
            </div>
          </div>
        </section>
        ...
        <section className={cn(styles.content, 'relative z-10 mb-3xl md:mb-6xl-1 lg:mb-6xl')}>
          <ResourceCard
            ...
            className={{
              wrapper: cn(
                'text-black',
                'lg:!grid-cols-2 lg:gap-x-0 lg:col-span-2',
                'md:!grid-cols-2 md:gap-x-0 md:col-span-2 md:px-xl',
                'sm:mx-auto w-full'
              ),
              ...
              link: 'text-primary text-12 py-4xs md:text-14 md:mt-auto lg:text-18 lg:py-xxs'
            }}
          >
