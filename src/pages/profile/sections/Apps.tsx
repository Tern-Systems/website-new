...
const AppsSection: FC = () => {
            ...
            return (
                ...
                >
                    {isFound ? (
                        <a
                            href={userApp?.link}
                            className={`col-start-2 capitalize ${styles.midCol} ${styles.ellipsis}`}
                            target={'_blank'}
                        >
                            {app}
                        </a>
                    ) : (
                        <span className={`col-start-2 capitalize ${styles.midCol} ${styles.ellipsis}`}>{app}</span>
                    )}
                    <Button
                        icon={icon}
                        hovered={{
                        ...
                        className={cn(
                            `col-start-3 flex-row-reverse place-self-end text-section-xs font-bold`,
                            styles.ellipsis,
                        )}
                        onClick={() => {
                            // TODO
    ...
    return (
        ...
        >
            ...
            <span className={`mt-[--p-content] ${styles.leftCol} ${styles.ellipsis}`}>Data Storage</span>
            <span className={`col-start-2 self-end text-section-xs ${styles.ellipsis}`}>Applications</span>
            {renderConnectedApps(DATA_STORAGE, userData.connectedApps.data)}

            <span className={`mt-[--p-content] ${styles.leftCol} ${styles.ellipsis}`}>Social Media</span>
            <span className={'col-start-2 self-end text-section-xs'}>Applications</span>
