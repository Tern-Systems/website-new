...
const OrderDetailsComponent: ForwardRefRenderFunction<HTMLDivElement, PropsWithoutRef<Props>> = (props: Props, ref: ForwardedRef<HTMLDivElement>) => {
    ...
    const Hr = <hr className={'border-white-d0 mt-xs mb-xs'}/>
    ...
    return (
        <div className={`${className} bg-white`}>
            <div ref={ref} className={'w-[min(100%,29rem)] place-self-center'}>
                ...
                {Hr}
                <div className={`flex flex-col gap-y-xxs mb-[min(8dvw,1.5rem)]`}>
                    ...
                    <span className={'text-secondary text-section'}>Qty {invoice?.item ? 1 : '--'}</span>
                </div>

                {Hr}
                <div className={'text-section-s font-bold text-secondary flex justify-between'}>
                    <span>Subtotal</span>
                ...
                {Hr}
                <div
                    className={`text-section-s font-bold text-secondary grid grid-cols-[1fr,max-content] gap-y-xxs`}>
                ...
                {Hr}
                <div className={'font-bold grid grid-cols-[1fr,max-content] gap-y-xxs'}>
                ...
                {Hr}
                <h3 className={'mt-[2.5rem] text-section-s font-bold text-secondary mb-[0.94rem] sm:mt-0'}>
                    Payment history
                </h3>
                <div className={'text-section grid grid-rows-2 gap-y-xxs'}>
