...
const FAQsPage: FC<Props> = (props: Props) => {
    ...
    const FAQsList = FAQs.map((faq, idx) => (
        <li key={faq.question + idx}>
            <Collapsible
                ...
                expandedState={[
                    expandedItemIdx === idx,
                    () => setExpandedItemIdx((prevState) => (prevState === idx ? -1 : idx)),
                ]}
                classNameWrapper={cn(
                    `[&]:p-0 rounded-none`,
                    `sm:[&]:portrait:px-xs`,
    ...
    return (
        <div className={cn(`h-full`, `sm:x-[px-s,pb-xl]`, `sm:landscape:pb-s`)}>
            <h1
                className={cn(
                    `block h-[5rem] content-end text-left font-bold`,
                    `text-heading`,
            ...
            >
                <ul
                    className={cn(
                        `overflow-y-scroll rounded-s bg-gray`,
                ...
                >
                    <span className={'mb-xxs block h-[--section-xs] font-bold'}>
                        {expandedItemIdx !== -1 ? FAQs[expandedItemIdx]?.question : null}
                    </span>
                    <span className={'block h-[calc(100%-var(--fz-section-xs)-var(--p-xxs))] overflow-y-scroll'}>
